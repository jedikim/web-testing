...
- Summary artifact: `runs/samples/artifacts/e2e-assistantless/2026-02-24/2026-02-24T03-54-58-642Z_assistantless-live-summary.json`
...
5. `assistantless_naver_weather_revise` (iteration 2/2)
   - status: pass
   - decisions: `revise`
6. `assistantless_daum_news_rulefirst` (iteration 2/2)
   - status: pass
7. `assistantless_sensitive_not_go_block` (iteration 2/2)
   - status: blocked
   - decisions: `not_go`
8. `assistantless_captcha_escalation_retry` (iteration 2/2)
   - status: pass
   - flow: captcha escalation retry success

### Notes

- 총 8개 시나리오(4종 x 2회) 모두 expected status와 actual status가 일치했다.
