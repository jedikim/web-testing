import json

from src.recon.knowledge_base import KnowledgeBase
from src.recon.models import GeneratedBundle, SiteProfile
from src.recon.runtime import ReconRuntime
from src.recon.validator import CodeValidator


class _BadCodegen:
    def generate_bundle(self, *, profile: SiteProfile, url: str, intent: str) -> GeneratedBundle:
        _ = profile, url, intent
        return GeneratedBundle(
            workflow_dsl={"schema_version": "1.0", "steps": []},
            prompts={"extract": "x"},
            strategy="dom_only",
            dependencies=["playwright"],
        )


def _profile() -> SiteProfile:
    return SiteProfile(
        domain="example.com",
        purpose="ecommerce",
        language="ko",
        region="KR",
        framework="react",
        is_spa=True,
        url_pattern="/search?query=*",
        content_types=["product_list"],
        repeating_patterns=["DIV>ARTICLE*12"],
        obstacle_types=[],
        navigation_hints=["category", "search"],
        interaction_hints=[],
        api_endpoints=["/api/search"],
        dom_hash="dom-1",
        ax_hash="ax-1",
    )


def test_runtime_does_not_promote_invalid_generated_bundle(tmp_path) -> None:
    kb = KnowledgeBase(base_dir=tmp_path)
    runtime = ReconRuntime(kb)

    out = runtime.execute_or_generate_stub(
        domain="example.com",
        url="https://example.com/search?q=tv",
        intent="find cheapest tv",
        profile=_profile(),
        codegen_agent=_BadCodegen(),
        validator=CodeValidator(),
    )

    assert out["status"] == "generation_failed"
    assert out["bundle_version"] is None

    patterns_root = tmp_path / "example.com" / "url_patterns"
    assert not patterns_root.exists()

    runs = (tmp_path / "example.com" / "history" / "runs.jsonl").read_text(encoding="utf-8")
    row = json.loads(runs.strip().splitlines()[-1])
    assert row["status"] == "generation_failed"
